...
if multiple_txt_files == "Yes":
    txt_files = st.file_uploader("Upload your text files", type=["txt"], accept_multiple_files=True)
    if txt_files:
...
# --- Process HCS Files ---
st.header("Step 2: Upload HCS Files")
multiple_hcs_files = st.radio("Do you have multiple HCS files to process?", ("Yes", "No"))

all_hcs_data = []

if multiple_hcs_files == "Yes":
    hcs_files = st.file_uploader("Upload your HCS files", type=["html"], accept_multiple_files=True)
    if hcs_files:
        for hcs_file in hcs_files:
            hcs_list = pd.read_html(hcs_file.read().decode('utf-8'))
            hcs = hcs_list[0]
    ...
    hcs_file = st.file_uploader("Upload your HCS file", type=["html"])
    if hcs_file:
        hcs_list = pd.read_html(hcs_file.read().decode('utf-8'))
        hcs_df = hcs_list[0]
...
if 'extracted_df' in locals() and 'hcs_df' in locals():
    ...
    output_dir = os.getcwd()  # Streamlit runs in current directory by default
    ...
    st.warning("Please upload both text and HCS files to proceed.")